...
/*
 * awardServiceRequestPointsIfEligible lived here and had no callers anywhere.
 *
 * It was the only code that read serviceRequestDailyMaxPoints, so the daily
 * cap it carefully enforced never applied to a single request. The live path
 * is syncServiceRequestPoints in guest-point-sync.ts, which now enforces the
 * cap itself, against the same Manila business day the NFC tap cap uses.
 */
